...
function Invoke-Phase18-FinalConfig {
    ...
    # ------------------------------------------------------------------
    # 0a. Ensure Central Administration IIS site is running
    #     After reboots the CA site may be stopped; force-start it.
    # ------------------------------------------------------------------
    Write-Log "Ensuring Central Administration IIS site is running..."
    try {
        Import-Module WebAdministration -ErrorAction SilentlyContinue

        # Ensure W3SVC (IIS) is running
        $w3svc = Get-Service W3SVC -ErrorAction SilentlyContinue
        if ($w3svc -and $w3svc.Status -ne 'Running') {
            Write-Log "Starting W3SVC (IIS) service..."
            Start-Service W3SVC -ErrorAction Stop
        }

        # Find and start the Central Admin site
        $caSite = Get-ChildItem IIS:\Sites -ErrorAction SilentlyContinue |
            Where-Object { $_.Name -like "*Central Administration*" }

        if ($caSite) {
            if ($caSite.State -ne 'Started') {
                Write-Log "Central Administration site is '$($caSite.State)' – starting..."
                Start-Website -Name $caSite.Name -ErrorAction Stop
                Write-Log "Central Administration site started"
            }
            else {
                Write-Log "Central Administration site is already running" -Level DEBUG
            }
        }
        else {
            Write-Log "Central Administration IIS site not found" -Level WARN
        }

        # Also ensure the SharePoint portal web app site (port 80) is started
        $portalSite = Get-ChildItem IIS:\Sites -ErrorAction SilentlyContinue |
            Where-Object { $_.Bindings.Collection -match $portalHost }

        if ($portalSite -and $portalSite.State -ne 'Started') {
            Write-Log "Portal site '$($portalSite.Name)' is '$($portalSite.State)' – starting..."
            Start-Website -Name $portalSite.Name -ErrorAction Stop
            Write-Log "Portal site started"
        }
    }
    catch {
        Write-Log "Failed to verify/start IIS sites: $_" -Level WARN
    }

    # ------------------------------------------------------------------
    # 0b. Disable LSA loopback check (BackConnectionHostNames)
    #     On single-server farms, NTLM auth fails for host-header sites
    #     that resolve to the local machine.  Without this, the portal
    #     web app returns 401 in a loop even with correct credentials.
    # ------------------------------------------------------------------
    Write-Log "Configuring BackConnectionHostNames for loopback authentication..."
    try {
        $lsaMsv1Path = "HKLM:\SYSTEM\CurrentControlSet\Control\Lsa\MSV1_0"
        $requiredHosts = @($portalHost, $env:COMPUTERNAME)

        $existingHosts = (Get-ItemProperty -Path $lsaMsv1Path -Name "BackConnectionHostNames" `
            -ErrorAction SilentlyContinue).BackConnectionHostNames

        # Merge existing + required, deduplicate
        $allHosts = @()
        if ($existingHosts) { $allHosts += $existingHosts }
        foreach ($h in $requiredHosts) {
            if ($allHosts -notcontains $h) { $allHosts += $h }
        }
        ...
        if (-not $existingHosts) {
            ...
            foreach ($h in $requiredHosts) {
                if ($existingHosts -notcontains $h) { $needsUpdate = $true; break }
            }
        }

        if ($needsUpdate) {
            Set-ItemProperty -Path $lsaMsv1Path -Name "BackConnectionHostNames" `
                -Value $allHosts -Type MultiString -Force
            Write-Log "BackConnectionHostNames set to: $($allHosts -join ', ')"
        }
        else {
            Write-Log "BackConnectionHostNames already configured" -Level DEBUG
        }
    }
    catch {
        Write-Log "Failed to configure BackConnectionHostNames: $_" -Level WARN
    }
